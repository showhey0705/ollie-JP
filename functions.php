...
/**
 * Open Props のアニメーション & イージングとテーマのスタイルを読み込む
 */
function enqueue_animation_assets() {
    // Open Props の animations と easing のみを利用
    wp_enqueue_style('open-props-animations', get_template_directory_uri() . '/assets/css/props.animations.css', array(), null);
    wp_enqueue_style('open-props-easing', get_template_directory_uri() . '/assets/css/props.easing.css', array(), null);
    
    // テーマのメインスタイル
    // wp_enqueue_style('theme-style', get_template_directory_uri() . '/assets/css/style.css', array(), wp_get_theme()->get('Version'));
}
add_action('wp_enqueue_scripts', __NAMESPACE__ . '\\enqueue_animation_assets');

/**
 * フロントエンド用のスクリプト（スクロール時のアニメーション発火）
 */
function enqueue_frontend_scripts() {
    wp_enqueue_script('theme-animations', get_template_directory_uri() . '/assets/js/animations.js', array('wp-dom'), null, true);
}
add_action('wp_enqueue_scripts', __NAMESPACE__ . '\\enqueue_frontend_scripts');

/**
 * Gutenberg エディター用のスクリプト（管理画面のみ）
 */
...
add_action('enqueue_block_editor_assets', function() {
    echo '<script>console.log("✅ functions.php で editor.js を読み込もうとしています！");</script>';
});
